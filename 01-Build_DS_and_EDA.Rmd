...
plot(CCMDs[,7:14])
```
<br>Right from the start, some of the data is redundant. Specifically, the 'med_sales_per_citizen' and 'rec_sales_per_citizen' variables which are calculated by dividing the county sales by the county population. 

```{r}
#subset out the two duplicative columns
CCMDs <- CCMDs[, -c(10:11)]
plot(CCMDs[,7:12])
```
