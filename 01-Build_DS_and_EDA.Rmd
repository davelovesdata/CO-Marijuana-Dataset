...
```
<br>Of course, it makes sense that population would correlate to sales and that med/rec sales would correlate so highly. 
...
```
<br> Four clusters seem to be immediately evident (low pop/low sales, med pop/low sales, high pop/low sales, and high pop/higher salesstarting around 1000K). Cluster analysis will need to be performed.

```{r}
#first scale and sequester the data fields of interest into a new dataframe. 
CCMDs_scale <- data.frame(scale(CCMDs[,7:9]))

#use kmeans to perform cluster identification
cluster <- kmeans(CCMDs_scale, centers=4, iter.max=50, nstart=5)
...
ggplot(data=CCMDs, aes(x = Population, color=cluster))+
  geom_point(shape=25, aes(y = Rec_Sales/100000), show.legend = TRUE, size = 1.25)+
  geom_point(shape=15, aes(y = Med_Sales/100000), show.legend = TRUE, size = 1.25)+
  labs(title="Medical and Retail Marijuana Sales as a measure of population", x= "Colorado County Population", y= "Marijuana Sales per $100K")+
  theme_bw()+
  coord_flip()
```
<br> The clusters didn't line up exactly as I expected but were grossly right. I'll try drawing ellipses around the clusters.

```{r}
ggplot(data=CCMDs, aes(x = Population, color=cluster))+
  geom_point(aes(y = Rec_Sales/100000), show.legend = TRUE, size = 1.25)+
  geom_smooth(aes(y=Rec_Sales/100000))+
  geom_point(aes(y = Med_Sales/100000), show.legend = TRUE, size = 1.25)+
  geom_smooth(aes(y=Med_Sales/100000))+
  labs(title="Medical and Retail Marijuana Sales as a measure of population", x= "Colorado County Population", y= "Marijuana Sales per $100K")+
  theme_bw()+
  coord_flip()
```
A lot of warnings that I need to disect. The cluster smothing pattern goes beyond -1 (I think) due to using the mean instead of the median. I think I can fix that. Also, I think a next step should be try the same exercise as above with Principal Component Analysis (PCA). I'm interested in the eigenvector values...
