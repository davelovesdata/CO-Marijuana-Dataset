...
#Replace NR with NA 
co_mj <- replace(co_mj, co_mj == "NR", NA)

#loop to convert sales/tax columns to numeric/currency
cashcol <- colnames(co_mj[8:14])

for (i in cashcol)
  {
  co_mj[[i]] <- as.numeric(co_mj[[i]])   #character to numeric
  
  co_mj[[i]] <- currency(co_mj[[i]], digits = 0L) #numeric but with symbology
  }
