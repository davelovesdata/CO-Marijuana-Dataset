...
```{r}
#test <- read_csv("https://storage.googleapis.com/co-publicdata/race-estimates-county.csv", col_names = TRUE, col_types = NULL)
#getwd()
#str(test)
...
#test <- spread(test, key="sex", value="count")
#test <- spread(test, key="sex", value="count")

#test <- test[, -c(1,5)]

#library(data.table)
#test <- setDT(test)[, lapply(.SD, function(x) unique(na.omit(x))), by = "county_fips"]
